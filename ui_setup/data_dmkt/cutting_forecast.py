...
class CuttingForecast:
    def __init__(self, check_delete):
    ...
    def get_data_web(self):
        try:
            ...
            if self.check_delete:
                if self.supa_func.truncate_table("cutting_forecast") == True:
                    print("✅ Đã xóa dữ liệu cutting forecast")
                else:
                    print("❌ Lỗi khi xóa dữ liệu cutting forecast")
                    return
            else:
                data_cutting = self.supa_func.get_data("cutting_forecast", "*")

                data_go = data_go[~data_go["SC_NO"].isin(data_cutting["GO"])] if not data_cutting.empty else data_go
            
            list_gos = data_go['SC_NO'].unique().tolist()
            total = len(list_gos)
            progress_bar = st.progress(0, text="Đang lấy dữ liệu Cutting Forecast...")
            
            data = []
            data_remaining = []
            # ========== Lặp qua từng GO ==========
            for idx, input_go in enumerate(list_gos):
                options = Options()
                ...
                progress_bar.progress((idx + 1) / total, text=f"Lấy dữ liệu GO: {input_go} ({idx+1}/{total})")
            progress_bar.empty()  # Ẩn progress bar sau khi xong
