...
class DmActual:
    ...
    def get_data(self):
        ...
        go_nos = data_go["SC_NO"].unique().tolist()
        go_nos_str = ','.join(f"'{go_no}'" for go_no in go_nos)

        data_fabric_trans = self.supa_func.get_data("fabric_trans", "*", f'"SC_NO" IN ({go_nos_str})')
        if data_fabric_trans.empty:
            print("❌ Không tìm thấy dữ liệu fabric_trans")
            return
        
        data_sm_trans = self.supa_func.get_data("submat_trans", "*", f'"SC_NO" IN ({go_nos_str})')
        if data_sm_trans.empty:
            print("❌ Không tìm thấy dữ liệu submat_trans")
            return
        
        data_wip = self.supa_func.get_data("process_wip", "*", f'"SC_NO" IN ({go_nos_str})')
        if data_wip.empty:
    ...
    def update_note_actual(self):
        ...
        df.to_excel("check_dm_actual.xlsx", index=False)
